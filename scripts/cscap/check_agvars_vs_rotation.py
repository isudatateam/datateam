...
for item in res["items"]:
    ...
    for entry2 in plotid_feed.entry:
        ...
        if "rotation" not in row:
            print(
                "Invalid headers in plotid sheet for %s\n headers: %s"
                % (item["title"], " ".join(row.keys()))
            )
            sys.exit()
        ...
        for entry in worksheet.list_feed.entry:
            ...
            for col in data.keys():
                ...
                if col not in cropmates.get(crop, []) and (
                    data[col] is None
                    or data[col].lower() in [".", "did not collect"]
                ):
                    print(
                        (
                            "    Setting to n/a PlotId: %s Crop: %s "
                            "Col: %s Data: %s"
                        )
                ...
                if data[col] is None:
                    ...
                    # entry.set_value(col, '.')
                    # dirty = True
                elif col not in cropmates.get(crop, []) and data[
                    col
                ].lower() not in [
                    ".",
                    "did not collect",
                    ...
                    print(
                        ("    PlotID: %s crop: %s has data [%s] for %s")
                        % (data["plotid"], crop, data[col], col.upper())
                    )
            if dirty:
                print("updating...")
