    ...
    for plotid in df:
        ...
        df[plotid]["valid"] = df[plotid][["Date", "Time"]].apply(
            lambda x: datetime.datetime.strptime(
                "%s/%s/%s %s" % (x[0].month, x[0].day, x[0].year, x[1]),
                "%m/%d/%Y %H:%M:%S",
            ),
            axis=1,
        )
        df[plotid]["depth"] = (
            df[plotid]["Water Table Depth below Ground"] * 10.0
        )
    return df
...
def process2(fn):
    ...
    for plotid in df:
        ...
        df[plotid]["depth"] = (
            df[plotid]["Level below ground"] * 0.3048 * 1000.0
        )
    return df
...
def process4(filename):
    ...
    for plotid in ["SW", "SE", "NW", "NE"]:
        res[plotid] = df[
            ["valid", "%s WAT4 Water Table Depth" % (plotid,)]
        ].copy()
        res[plotid].columns = ["valid", "depth"]
        # shrug/sigh
        ...
        res[plotid]["depth"] = (
            pd.to_numeric(res[plotid]["depth"], errors="coerce") * 10.0
        )
    return res
