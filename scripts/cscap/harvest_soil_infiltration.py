...
res = drive_client.files(
        ).list(q="title contains 'Soil Infiltration Data'").execute()
...
for item in res['items']:
    ...
    if (worksheet.get_cell_value(1, 1) != 'plotid' or
        ...
        print(('harvest_soil_infiltration %s[%s] headers: "%s","%s", skipping'
    ...
    for row in range(4, worksheet.rows+1):
        ...
        if depth.find(" to ") == -1:
            print(("harvest_soil_infiltration found invalid depth: %s %s %s"
                   ) % (depth, siteid, YEAR))
            continue
        ...
        for col in range(3, worksheet.cols+1):
            if worksheet.get_cell_value(1, col) is None:
                print(('harvest_soil_infiltration Year: %s Site: %s Col: %s is null'
                       ) % (YEAR, siteid, col))
                continue
            ...
            if inval is not None and val is None:
                print(("harvest_soil_infiltration found None. site: %s year: %s "
