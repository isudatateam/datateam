...
def main():
    ...
    for entry in sheet.get_list_feed().entry:
        ...
        if row['operation'] != 'fertilizer_synthetic':
            continue
        if (row['productrate'] is None or row['productrate'] == '-1.0' or
                row['productrate'] == ''):
            option = 'B'
            ...
            nitrogen = prate * float2(row['nitrogen']) / 100.
            phosphoruse = 0.
            potassium = 0.
            if row['phosphorus'] is not None and float2(row['phosphorus']) > 0:
                phosphoruse = prate * float2(row['phosphorus']) / 100.
            if row['phosphate'] is not None and float2(row['phosphate']) > 0:
                phosphoruse = prate * float2(row['phosphate']) / 100. * 0.437
            if row['potassium'] is not None and float2(row['potassium']) > 0:
                potassium = prate * float2(row['potassium']) / 100.
            if row['potash'] is not None and float2(row['potash']) > 0:
                potassium = prate * float(row['potash']) / 100. * 0.830
            sulfur = prate * float2(row['sulfur']) / 100.
            zinc = prate * float2(row['zinc']) / 100.
            magnesium = prate * float2(row['magnesium']) / 100.
            calcium = prate * float2(row['calcium']) / 100.
            iron = prate * float2(row['iron']) / 100.
