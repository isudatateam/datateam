...
def process6(uniqueid, dirname):
    ...
    res = {}
    rename = {
        "Port 1 VWC": "d1moisture",
        "Port 1 Temp": "d1temp",
        "Port 2 VWC": "d2moisture",
        "Port 2 Temp": "d2temp",
        "Port 3 VWC": "d3moisture",
        "Port 3 Temp": "d3temp",
        "Port 4 VWC": "d4moisture",
        "Port 4 Temp": "d4temp",
        "Port 5 VWC": "d5moisture",
        "Port 5 Temp": "d5temp",
    }
    for fn in files:
        ...
        df2[plotid] = pd.DataFrame(
            {
                ...
                "d4moisture": (
                    df["%s_d4moisture" % (plotid,)]
                    if "%s_d4moisture" % (plotid,) in df.columns
                    else None
                ),
                "d5moisture": (
                    df["%s_d5moisture" % (plotid,)]
                    if "%s_d5moisture" % (plotid,) in df.columns
                    else None
                ),
            }
        )
