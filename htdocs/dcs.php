...
{
    return sprintf(
        "<th>%s:</th><td><input type=\"text\" name=\"%s\" width=\"%s\" value=\"%s\"></td>",
        $label,
        $varname,
        $inputwidth,
        get_field($varname)
...
{
    return sprintf(
        "<th class='tdnowrap'>%s</th><td><input type=\"radio\" name=\"%s\" value=\"yes\">Yes <input type=\"radio\" name=\"%s\" value=\"no\">No</td>",
        $label,
        $varname,
        $varname
    );
...
{
    return sprintf(
        "<td><input type=\"radio\" name=\"%s\" value=\"yes\"> Yes <input type=\"radio\" name=\"%s\" value=\"no\"> No </td>",
        $varname,
        $varname
    );
}
  ...
  <li><a href=\"#field${field}_tabs3\">Rotation Three</a></li>
                ...
                for ($rotation = 1; $rotation < 4; $rotation++) {
                    ...
                    echo "<th>Harvest:</th><td>" . radio("f${field}r${rotation}harvest", array(
                        ...
                        "grain_and_res" => "Grain and residue"
                    )) . "</td>";
                    echo "</tr></table>";
  ...
  <li><a href=\"#field${field}_ctabs3\">Cover Crop Three</a></li>
            ...
            <table cellpadding='2' cellspacing='0' border='1'>
                ...
                foreach ($conservation as $k => $v) {
                    ...
                    echo sprintf(
                        "<tr style='background: ${color}'><th>%s</th>
    %s %s %s
    </tr>",
                        $v,
                        tdyn2("field1conserv${k}"),
                        tdyn2("field2conserv${k}"),
